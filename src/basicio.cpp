    ...
    byte* FileIo::mmap(bool isWriteable)
    {
        ...
        if (munmap() != 0) {
            ...
            if (p_->wpMode_ == Impl::wpUnicode) {
                throw WError(ErrorCode::kerCallFailed, wpath(), strError().c_str(), "munmap");
            }
            else
#endif
            {
...
#ifdef EXV_UNICODE_PATH
            if (p_->wpMode_ == Impl::wpUnicode) {
                throw WError(ErrorCode::kerFailedToMapFileForReadWrite, wpath(), strError().c_str());
            }
            else
#endif
            {
...
#ifdef EXV_UNICODE_PATH
            if (p_->wpMode_ == Impl::wpUnicode) {
                throw WError(ErrorCode::kerCallFailed, wpath(), "MSG1", "_get_osfhandle");
            }
            else
#endif
            {
...
#ifdef EXV_UNICODE_PATH
            if (p_->wpMode_ == Impl::wpUnicode) {
                throw WError(ErrorCode::kerCallFailed, wpath(), "MSG2", "DuplicateHandle");
            }
            else
#endif
            {
...
#ifdef EXV_UNICODE_PATH
            if (p_->wpMode_ == Impl::wpUnicode) {
                throw WError(ErrorCode::kerCallFailed, wpath(), "MSG3", "CreateFileMapping");
            }
            else
#endif
            {
...
#ifdef EXV_UNICODE_PATH
            if (p_->wpMode_ == Impl::wpUnicode) {
                throw WError(ErrorCode::kerCallFailed, wpath(), "MSG4", "CreateFileMapping");
            }
            else
#endif
            {
...
#ifdef EXV_UNICODE_PATH
                if (p_->wpMode_ == Impl::wpUnicode) {
                    throw WError(ErrorCode::kerFileOpenFailed, wpath(), "a+b", strError().c_str());
                }
                else
#endif
                {
                ...
                if (hKernel) {
                    ...
                    if (pfcn_ReplaceFileW) {
                        ...
                        if (ret == 0) {
                            if (GetLastError() == ERROR_FILE_NOT_FOUND) {
                                if (::_wrename(fileIo->wpath().c_str(), wpf) == -1) {
                                    throw WError(ErrorCode::kerFileRenameFailed, fileIo->wpath(), wpf, strError().c_str());
                                }
                                ::_wremove(fileIo->wpath().c_str());
                            }
                            else {
                        ...
                        if (fileExists(wpf) && ::_wremove(wpf) != 0) {
                            throw WError(ErrorCode::kerCallFailed, wpf, strError().c_str(), "::_wremove");
                        }
                        if (::_wrename(fileIo->wpath().c_str(), wpf) == -1) {
                            throw WError(ErrorCode::kerFileRenameFailed, fileIo->wpath(), wpf, strError().c_str());
                        }
...
#ifdef EXV_UNICODE_PATH
                if (p_->wpMode_ == Impl::wpUnicode) {
                    throw WError(ErrorCode::kerFileOpenFailed, wpath(), "w+b", strError().c_str());
                }
                else
#endif
                {
...
#ifdef EXV_UNICODE_PATH
                if (p_->wpMode_ == Impl::wpUnicode) {
                    throw WError(ErrorCode::kerDataSourceOpenFailed, src.wpath(), strError().c_str());
                }
                else
#endif
                {
...
#ifdef EXV_UNICODE_PATH
                if (p_->wpMode_ == Impl::wpUnicode) {
                    throw WError(ErrorCode::kerFileOpenFailed, wpath(), lastMode.c_str(), strError().c_str());
                }
                else
#endif
                {
...
#ifdef EXV_UNICODE_PATH
            if (p_->wpMode_ == Impl::wpUnicode) {
                throw WError(ErrorCode::kerTransferFailed, wpath(), strError().c_str());
            }
            else
#endif
            {
